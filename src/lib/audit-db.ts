...
if (jsonOut) {
  ...
  console.log(
    `ENTRAIN database soundtrack audit · ${new Date(report.generatedAt).toISOString()}`,
  );
  console.log(
    `Rows ${report.totals.rows} · OK ${report.totals.ok} · WARN ${report.totals.warn} · FAIL ${report.totals.fail} · gated ${report.totals.gated}`,
  );
  for (const row of report.rows) {
    ...
    if (row.referenceMatch)
      console.log(
        `  reference: ${row.referenceMatch.referenceId} · ${row.referenceMatch.matches ? "matches" : "differs"} · score ${row.referenceMatch.score}/100`,
      );
