      ...
      <div className="toolbar">
        ...
        <div className="tagrow">
          <button
            ...
            onClick={async () => run(connectAndVerify)}
          >
            Connect Phantom
          </button>
          <button
            className="btn"
            onClick={async () => run(refreshWalletBalance)}
          >
            Refresh balance
          </button>
        </div>
...
async function run(fn: () => Promise<WalletState>) {
  ...
  try {
    ...
    message = e.message || "wallet check failed";
  }
  paint();
}
...
function markCards() {
  document
    .querySelectorAll<HTMLElement>("[data-soundtrack-card]")
    .forEach((el) => {
