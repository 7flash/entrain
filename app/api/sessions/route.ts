...
type Body = { slug?: string; sourceSlug?: string; name?: string; description?: string; tags?: string[]; session?: any };

export async function POST(req: Request) {
  const auth = authFromRequest(req);
  const access = decideLibraryAccess(auth, 'save');
  if (!access.ok || !auth) return json({ ok: false, error: access.message, requiresWallet: true }, { status: 401 });
  ...
  const row = db.savedSessions.insert({
    ...
    description: body.description ? String(body.description).slice(0, 1000) : session.description || session.notes?.slice(0, 1000),
  ...
  try { db.playEvents.insert({ publicKey: auth.publicKey, savedSessionId: Number((row as any).id || 0), action: 'save', createdAt: Date.now() }); } catch {}
...
export function GET(req: Request) {
  ...
  if (!access.ok || !auth) return json({ ok: false, error: access.message, requiresWallet: true }, { status: 401 });
  const rows = db.savedSessions.select().where({ publicKey: auth.publicKey }).orderBy('createdAt','DESC').limit(100).all();
