...
type Body = { name?: string; description?: string; tags?: string[]; session?: any; isFavorite?: boolean; lastPlayedAt?: number };

function owned(id: string, publicKey: string) {
  return db.savedSessions.select().where({ id: Number(id), publicKey }).first() as any;
}

export async function PATCH(req: Request, { params }: Props) {
  const auth = authFromRequest(req);
  const access = decideLibraryAccess(auth, 'update');
  if (!access.ok || !auth) return json({ ok:false, error: access.message }, { status: 401 });
  const row = owned(params.id, auth.publicKey);
  if (!row) return json({ ok:false, error:'saved track not found' }, { status: 404 });
  ...
  db.savedSessions.update(patch).where({ id: Number(params.id), publicKey: auth.publicKey }).run();
...
export async function DELETE(req: Request, { params }: Props) {
  ...
  const row = owned(params.id, auth.publicKey);
  if (!row) return json({ ok:false, error:'saved track not found' }, { status: 404 });
  db.savedSessions.delete().where({ id: Number(params.id), publicKey: auth.publicKey }).run();
  return json({ ok:true });
}
